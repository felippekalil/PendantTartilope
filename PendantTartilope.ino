...
void tratamento(const byte c)
{
	...
	case '<':
	case 144:
		lcd.command(0x10); // left shift cursor
		break;
	case '>':
	case 148:
		lcd.command(0x14); // right shift cursor
		break;
	case '&':
		pegaBotao = true;
		break;
...
void interpretaDados()
{
	...
	{
#ifdef MODO_ECO
		Serial.write(bufferAcc[0]);
#endif
		tratamento(bufferAcc[0]);
		removeBuffer();
	}
}
